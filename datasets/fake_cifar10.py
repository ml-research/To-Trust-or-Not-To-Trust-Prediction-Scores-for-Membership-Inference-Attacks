...
class FakeCIFAR10(ImageFolder):
    ...
    image_folder_md5_for_10k_imgs = 'bca9b625c927281f07ab838fcf647d28'
    ...
    def _check_integrity(self):
        # check the integrity of the image folder but only if there are supposed to be 10k samples
        if self.num_generated_samples == 10000 and not self._check_folder_integrity(
            os.path.join(self.base_folder, "Images"), self.image_folder_md5_for_10k_imgs
        ):
            return False

        # check the integrity of the pre-trained network
        if self.num_generated_samples != 10000 and not check_integrity(
            os.path.join(self.base_folder, self.pretrained_net_filename), self.pretrained_net_md5
        ):
            return False
    ...
    def generate_samples(self) -> None:
        ...
        for latent_code_chunk, class_idx in tqdm(zip(latent_codes, [i for i in range(10)]), desc='Generating images',
                                                 total=len(latent_codes)):
            label = torch.zeros([1, generator.c_dim]).to(device)
            label[:, class_idx] = 1
            imgs = []
            for code in latent_code_chunk:
                code = code.to(device)
                # generate the images
                img = generator(code.unsqueeze(0), label)[0].cpu()
                imgs.append(img)
            self._save_images(imgs, os.path.join(self.base_folder, "Images", cifar10_classes[class_idx]))
